...
class Database:    
    ...
    async def movie_update_status(self, bot_id):
        return await self.get_bot_setting(bot_id, 'UPDATE_NOTIFICATION', UPDATE_NOTIFICATION)
    ...
    async def increment_file_count(self, user_id):
        ...
        if not last_reset or last_reset < today_reset:
            ...
            await self.col.update_one(
                {"id": user_id},
                {"$set": {"file_count": new_count, "file_reset_time": today_reset.isoformat()}}
            )
        else:
            new_count = user.get("file_count", 0) + 1
            await self.col.update_one(
                {"id": user_id},
    ...
    async def get_user_file_count(self, user_id):
        ...
        if not last_reset or last_reset < today_reset:
            await self.col.update_one(
                {"id": user_id},
                {"$set": {"file_count": 0, "file_reset_time": today_reset.isoformat()}}
            )
            return 0
        return user.get("file_count", 0)

    async def reset_file_count(self, user_id=None):
        now = datetime.datetime.now(TIMEZONE)
        now_str = now.isoformat()
        if user_id:
            await self.col.update_one(
                {"id": user_id},
                {"$set": {"file_count": 0, "file_reset_time": now_str}}
            )
        else:
    ...
    async def get_remaining_files(self, user_id, limit):
        if await self.has_premium_access(user_id):
            return "∞"
        count = await self.get_user_file_count(user_id)
        return max(0, limit - count)

    async def get_time_until_reset(self, user_id):
        now = datetime.datetime.now(TIMEZONE)
